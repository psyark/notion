...
func TestProperty(t *testing.T) {
	...
	c.ExpectBlock(&Block{
		...
		jsonStr := rewriteInaccurateExampleJSON_AddDescription(fmt.Sprintf(`{%s}`, e.Text), "Store availability", "multi_select", "options")
...

	{
		...
		c.ExpectBlock(&Block{
			...
			jsonStr := rewriteInaccurateExampleJSON_AddDescription(fmt.Sprintf(`{%s}`, e.Text), "Food group", "select", "options")
			converter.AddUnmarshalTest("PropertyMap", jsonStr)
		})
...

	{
		...
		c.ExpectBlock(&Block{
			...
			jsonStr := rewriteInaccurateExampleJSON_AddDescription(fmt.Sprintf(`{%s}`, e.Text), "Status", "status", "options")
			converter.AddUnmarshalTest("PropertyMap", jsonStr)
		})
		c.ExpectBlock(&Block{
			Kind: "Blockquote",
