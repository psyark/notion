...
func (c *Converter) OutputBindingHelper() {
	...
	helper.ImportNames(map[string]string{
		"github.com/psyark/notion": "notion",
	})
	helper.HeaderComment("Code generated by notion.doc2api; DO NOT EDIT.")
	helper.Func().Id("getTypeForBinding").Params(jen.Id("p").Qual("github.com/psyark/notion", "Property")).String().Block(
	...
	helper.Func().Id("getPayload").Params(jen.Id("p").Op("*").Qual("github.com/psyark/notion", "PropertyValue")).Params(jen.Any(), jen.Error()).Block(
		jen.Switch(jen.Id("p").Dot("Type")).BlockFunc(func(g *jen.Group) {
			for _, f := range propertyValue.fields {
				if f, ok := f.(*VariableField); ok && f.discriminatorValue == f.name {
	...
	helper.Func().Id("setPayload").Params(jen.Id("p").Op("*").Qual("github.com/psyark/notion", "PropertyValue"), jen.Id("value").Any()).Error().Block(
		jen.Switch(jen.Id("p").Dot("Type")).BlockFunc(func(g *jen.Group) {
			for _, f := range propertyValue.fields {
				if f, ok := f.(*VariableField); ok && f.discriminatorValue == f.name {
